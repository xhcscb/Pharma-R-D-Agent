...
- 可筛选的实体关系图，包括实体类型、提及次数、关系类型和重复证据数量；
- PDF 表格中的字符级实体高亮、规范名称、字符区间和实体链接状态；
- 许可状态、访问等级、任务状态和冲突数量。
...
| `relation_graph` | 候选实体节点、聚合关系边、自环标记、页码和证据片段 |
| `entity_extraction_example` | 文档元素、bbox、原文、字符区间和实体链接样例 |
...
关系图默认隐藏自环关系。打开“显示自环候选”可以检查由实体消歧或同一表格多次提及造成的明显错误。边宽表示合并到同一主体、关系和客体组合的候选证据数量，不表示关系更可信。

实体抽取视图按字符区间高亮原文。切换实体类型后，可查看药物、靶点、适应症、临床阶段和 PipelineProgram 如何从同一表格元素中识别并链接到规范实体。页面、bbox、解析器版本和置信度共同构成定位信息。
