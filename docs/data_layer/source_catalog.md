...
支持范围固定为 `CN-MAINLAND`。完整的 34 项来源、域名白名单、访问方式、样本和再分发规则见
...
| 证券行情 | 上证信息、深圳证券信息、中证股转科技或获许可终端 | 授权 JSON/CSV/XLSX 清单 | `authorized_restricted` |
...
- 公众可访问不等于允许公开复制，官网材料默认 `public_access + restricted`；
